        ...
        <div className="w-full flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between"> 
                <div className="flex items-center gap-2"> 
                    <Label> 限界 | Limit </Label>
                    <Select value={currentLimit.toString()} onValueChange={(value) => onChangeLimit(Number(value))}>
                    <SelectTrigger> 
                        <SelectValue placeholder="制限の選択 | Select Limit" /> 
                    </SelectTrigger>
                    <SelectContent>
                        <SelectGroup>
                            <SelectLabel> 限界 | Limit </SelectLabel>
                            {LIMIT_LISTS.map((currentLimit) => (
                            <SelectItem key={currentLimit} value={currentLimit!.toString()}> 
                            {currentLimit}
                            </SelectItem>
                        ))}
                        </SelectGroup>
