...
import { DEFAULT_LIMIT } from '@/constants/data-table-constant';
  ...
  const filteredData = useMemo(() => {
    return (menus?.data || []).map((menu: Menu, index) => {
      return [
        ...
        menu.category,
        <div>
          <p>Base: {convertJPY(menu.price)}</p>
          <p>Discount: {menu.discount}</p>
          <p>
            After Discount:{' '}
            {convertJPY(menu.price - (menu.price * menu.discount) / 100)}
          </p>
        </div>,
        <div
          className={cn(
  ...
  return (
    <div className="w-full">
      <div className="flex flex-col lg:flex-row mb-4 gap-2 justify-between w-full">
        <h1 className="text-2xl font-bold"> メニュー管理 | Menu Management</h1>
